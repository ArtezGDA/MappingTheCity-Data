...
function getHourArray() {
	// JSON reading
	var fileToRead = File("/Users/Nikki/Documents/A School/Artez GD2A/Media Design/Illustrator/hourOccrences-manhattan-ASSAULT.json");
	var jsonData = null;
	if ( fileToRead !== false ) {
...
if ( app.documents.length > 0 ) {
	...
	if ( textObjects.length > 0 ) {
		...
		if ( textObjects.length == hours.length ) {
			
			for (var i = 0; i < textObjects.length; i++) {
				var textObj = textObjects[textObjects.length - (i + 1)];
			
				// Change the text into the month
				var thisHour = hours[i];
				textObj.contents = thisHour.hour;
				...
				var adjustment = thisHour.count / 100.0;
			...
			alert("the number of textFrames and number of hours in the JSON do not match.");
		}
	}
}